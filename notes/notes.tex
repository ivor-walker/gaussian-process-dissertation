...
% \begin{itemize}
%     \item Our covariance functions has some hyperparameters, e.g. the full form of SE in one dimension contains some free parameters 
% \begin{equation}
%     k_y(x,x') = \sigma^2_f \exp\left(-\frac{1}{2}\frac{|x - x'|^2}{l^2}\right) + \sigma^2_n\delta_{x,x'}
% \end{equation}
%     \begin{itemize}
%         \item Note that our covariance function is for $k_y$ as it is for the noisy targets $y$, not the function $f$
%         \item $\sigma^2_f$ is the signal variance, which controls the overall scale of the function
%         \item $\sigma^2_n$ is the noise variance, which controls the amount of noise in the observations
%         \item $l$ is the length scale, which controls how quickly the covariance decays with distance
%         \item $\delta_{X,X'}$ is the Kronecker delta which enforces independence of noise
%     \end{itemize}
% \end{itemize}
